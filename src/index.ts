...
function printHelp() {
  ...
  const help = [
    ...
    "  mcp-ssh-agent enroll --server <url> --token <token> --alias <alias>",
    "  mcp-ssh-tool agent run   Run the no-custody outbound agent",
    "  mcp-ssh-agent run        Run the no-custody outbound agent",
    "  mcp-ssh-tool agent status Show local agent enrollment status",
    "  mcp-ssh-agent status     Show local agent enrollment status",
